#!/usr/bin/python3
# -*- coding: utf-8 -*-
# Author: Annette Rios, Rico Sennrich, Anonymous Author
...
def count_errors(
    ...
    if results_by_d:
        fres0 = open(results_file + "0", 'w+')
        fres1 = open(results_file + "1", 'w+')
        fres2 = open(results_file + "2", 'w+')
        fres3 = open(results_file + "3", 'w+')
        fres4 = open(results_file + "4", 'w+')
    else:
    ...
    for count, sentence in enumerate(reference):
        ...
        if results_by_d:
            if ante_dist == 0:
                fres0.write(str(int(all_better)) + '\n')
            elif ante_dist == 1:
                fres1.write(str(int(all_better)) + '\n')
            elif ante_dist == 2:
                fres2.write(str(int(all_better)) + '\n')
            elif ante_dist == 3:
                fres3.write(str(int(all_better)) + '\n')
            elif ante_dist == ">3":
                fres4.write(str(int(all_better)) + '\n')
        else:
            fres.write(str(int(all_better)) + '<SRC>' + sentence["src segment"] + '<SRC>' + sentence["ref segment"] + '\n')
...
def main(reference, scores, maximize, out, verbose, results_by_d):

    results = count_errors(reference, scores, maximize, out, verbose, results_by_d )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--results-by-d', action="store_true", help="write prediction results in different files according to the distance of the antecedent")
    ...
    parser.add_argument('--results-file', '-rf',
                        metavar='PATH',
                        help="File to write the result for each contrastive block")
