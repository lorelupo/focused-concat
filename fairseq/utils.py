...
def post_process_prediction(
    ...
    hypo_str = tgt_dict.string(
        hypo_tokens, remove_bpe, extra_symbols_to_ignore=extra_symbols_to_ignore, include_eos=include_eos,
    )
...
def move_to(obj, device):
    ...
    Args:
        obj [Tensor or dict or list]
        device [str]: name of the device. E.g., cpu or cuda:0.
    """
...
def sort_back(t, sort_order):
    """
    Sort back to original order a Tensor t
    that had been previously sorted according to sort_order.
    """
    _, ord = sort_order.sort()
    return t.index_select(0, ord)

def pad_smaller(a, b, dim=0, append_padding=False, value=1):
    """
    Given two tensors a and b with a different size along dimension dim, it pads the smallest one to match the size and returns the two tensors where one of the two have been padded.
    Args:
        a (Tensor)
        b (Tensor): of the same type and the same number of dimensions as a
        dim (int): the dimension to pad
        append_padding (bool): if True, append padding, else prepend it
        value (int): padding value
    Out:
        a (Tensor): padded if it was smaller compared to b along dimension dim
        b (Tensor): padded if it was smaller compared to a along dimension dim
        which(int, optional): 0 if a was padded, 1 if b was padded, else None
    """
    import torch.nn.functional as F
    assert len(a.shape) == len(b.shape)
    assert dim <= len(a.shape)

    pad_size = a.shape[dim] - b.shape[dim]
    which=None

    if pad_size != 0:
        pad = [0] * len(a.shape) * 2
        pad[-(dim+1)*2+int(append_padding)] = abs(pad_size)
        if pad_size > 0:
            b = F.pad(input=b, pad=tuple(pad), mode='constant', value=value)
            which = 1
        elif pad_size < 0:
            a = F.pad(input=a, pad=tuple(pad), mode='constant', value=value)
            which = 0
