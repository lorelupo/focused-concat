...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(
        self, src_tokens, src_lengths, return_all_hiddens: bool = False
    ):
        """
        Args:
    ...
    def max_positions(self):
        ...
        return min(
            self.max_source_positions, self.embed_positions.max_positions
        )
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        # project embeddings to match dimensions
        self.project_out_dim = (
            Linear(embed_dim, self.output_embed_dim, bias=False)
            if embed_dim != self.output_embed_dim and
            not args.tie_adaptive_weights else None
        )
    ...
    def extract_features_scriptable(
        ...
        if self.cross_self_attention or prev_output_tokens.eq(self.padding_idx
                                                             ).any():
    ...
    def max_positions(self):
        ...
        return min(
            self.max_target_positions, self.embed_positions.max_positions
        )
    ...
    def buffered_future_mask(self, tensor):
        ...
        if (
            ...
            (not self._future_mask.device == tensor.device) or
...
@register_model_architecture('transformer', 'transformer_iwslt_fr_en')
...
@register_model_architecture('transformer', 'transformer_test')
def transformer_test(args):
    ...
    args.decoder_ffn_embed_dim = getattr(args, 'decoder_ffn_embed_dim', 100)
    args.decoder_attention_heads = getattr(args, 'decoder_attention_heads', 2)
    args.decoder_layers = getattr(args, 'decoder_layers', 2)
    base_architecture(args)

@register_model_architecture("transformer", "transformer_vaswani_wmt_en_fr")
...
@register_model_architecture("transformer", "transformer_voita_fairseq")
def transformer_voita_fairseq(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 2048)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 8)
    args.encoder_layers = getattr(args, "encoder_layers", 6)
...
@register_model_architecture("transformer", "transformer_voita")
def transformer_voita(args):
    # num layers
    args.encoder_layers = getattr(args, "encoder_layers", 6)
    args.decoder_layers = getattr(args, "decoder_layers", 6)
    # num heads
    ...
    # ffn_type
    args.activation_fn = getattr(args, "activation_fn", "relu") # conv_relu equivalent
    # hid_size=emb_size
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.decoder_embed_dim = getattr(args, "decoder_embed_dim", args.encoder_embed_dim)    
    # equivalent of res_steps=nlda
    args.encoder_normalize_before = getattr(args, "encoder_normalize_before", False)
    args.decoder_normalize_before = getattr(args, "decoder_normalize_before", False)
    # rescale emb
    args.no_scale_embedding = getattr(args, "no_scale_embedding", False)
    # inp_emp_bias is unavailable
    # normalize_out is by default
    # don't share emb
    args.share_all_embeddings = getattr(args, "share_all_embeddings", False)
    # relu dropout
    args.activation_dropout = getattr(args, "activation_dropout", 0.1)
    ...
    args.dropout = getattr(args, "dropout", 0.1)
    # attn dropout
    args.activation_dropout = getattr(args, "attention_dropout", 0.1)
    base_architecture(args)

@register_model_architecture("transformer", "transformer_base_extra_dropout")
def transformer_base_extra_dropout(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 2048)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 8)
    args.encoder_layers = getattr(args, "encoder_layers", 6)
