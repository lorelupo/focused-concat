    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args):
        ...
        self.fc1 = self.build_fc1(
            self.embed_dim, args.encoder_ffn_embed_dim, self.quant_noise,
            self.quant_noise_block_size
        )
        self.fc2 = self.build_fc2(
            args.encoder_ffn_embed_dim, self.embed_dim, self.quant_noise,
            self.quant_noise_block_size
        )
    ...
    def forward(
        ...
        if not self.normalize_before:
            x = self.final_layer_norm(x)
        if need_attn:
            # if/else needed no to break calls requiring a single return value
            return x, attn
        else:
            return x
...
class TransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(
        ...
        self.fc1 = self.build_fc1(
            self.embed_dim, args.decoder_ffn_embed_dim, self.quant_noise,
            self.quant_noise_block_size
        )
        self.fc2 = self.build_fc2(
            args.decoder_ffn_embed_dim, self.embed_dim, self.quant_noise,
            self.quant_noise_block_size
        )
    ...
    def forward(
                ...
                self_attn_mask = torch.cat(
                    (
                        x.new_zeros(x.size(0),
                                    encoder_out.size(0)), self_attn_mask
                    ),
                    dim=1
                )
        ...
        # anything in the original self_attn_mask = 1 becomes -1e8 and self_attn_mask = 0 becomes 0
        # to prevent NaN in model parameters (see same check in TransformerEncoderLayer)
        if self_attn_mask is not None:
            self_attn_mask = self_attn_mask.masked_fill(
                self_attn_mask.to(torch.bool),
                -1e8 if x.dtype == torch.float32 else -1e4
            )
        ...
        if self.onnx_trace and incremental_state is not None:
            ...
            return x, self_attn if need_self_attn else attn, self_attn_state
        return x, self_attn if need_self_attn else attn, None
