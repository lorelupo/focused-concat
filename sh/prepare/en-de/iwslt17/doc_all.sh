#!/usr/bin/env bash
#
# IWSLT17 train data, test2011-2014  as  dev  set, test2015 as test set
#
# Possible usages:
#
# bash sh/prepare/en-de/iwslt17/doc_all.sh standard
# bash sh/prepare/en-de/iwslt17/doc_all.sh shuffled
# bash sh/prepare/en-de/iwslt17/doc_all.sh split
# bash sh/prepare/en-de/iwslt17/doc_all.sh 3split
# bash sh/prepare/en-de/iwslt17/doc_all.sh zero-shot-split
# bash sh/prepare/en-de/iwslt17/doc_all.sh aligned-split
# bash sh/prepare/en-de/iwslt17/doc_all.sh synt-split
# bash sh/prepare/en-de/iwslt17/doc_all.sh syntall-split
...
then
  # This shuffled option will work only if the standard dataset already exists
  # and has been processed with the code above (otion "standard")
  standard_test=$corpus/standard/test.$src
  if [ ! -f "$standard_test" ]; then
    echo "$standard_test not found."
...
then
  ...
  for f in train valid ; do
      python ../../scripts/split_corpus_sentences.py \
        --src=$tmp/$f.$src --tgt=$tmp/$f.$tgt --min-length 7 15
      rm -f $tmp/$f.$src
      rm -f $tmp/$f.$tgt
      mv $tmp/$f.$src.split $tmp/$f.$src
      mv $tmp/$f.$tgt.split $tmp/$f.$tgt
...
then
  ...
  echo "Retrieving documents' heads..."
  for l in $src $tgt; do
    for f in train valid; do
      # generate heads
...
then
  ...
  for f in train valid; do
      python ../../scripts/split_corpus_sentences.py \
        --src=$tmp/$f.$src --tgt=$tmp/$f.$tgt --coref-info=$corpus/coref_info/$fcoref.$lang.$f --verbose=1 \
        | tee $tmp/splitting_$f.logs
      rm -f $tmp/$f.$src
      rm -f $tmp/$f.$tgt
      mv $tmp/$f.$src.split $tmp/$f.$src
