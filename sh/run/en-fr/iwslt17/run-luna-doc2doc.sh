...
# Luna model for the translation task, on a given corpus and
...
this_script=/home/getalp/alonzocl/dev/scripts/en-fr/run-luna-doc2doc.sh
...
# Luna
if [ -n "$proj_len" ]; then proj_len=$proj_len; else proj_len=16; fi
if [ -n "$arch" ]; then arch=$arch ; else arch=luna_base ; fi
# Train
...
then
    ...
    d=anaphora
    bash $this_script --t=score-ref --src=$src --tgt=$tgt \
    --data-dir=$test_suites/$d --save_dir=$save_dir \
    --mode=$mode --opt=$opt --val=$val --testlog=$d
