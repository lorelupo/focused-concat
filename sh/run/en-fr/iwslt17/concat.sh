...
then
    ...
    for d in lexical_choice anaphora; do
        # score reference
        bash $this_script --t=score-ref --save_dir=$save_dir --data_dir=$test_suites/$d --testlog=$d --cuda=$cuda --mover=$mover --mode=$mode --opt=$opt --val=$val --scored_checkpoint=$scored_checkpoint --batch_size=32 --need_seg_label=$need_seg_label        # --mover="{'n_context_sents':'1'}"
        # evaluate
