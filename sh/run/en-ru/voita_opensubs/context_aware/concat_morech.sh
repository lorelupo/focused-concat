...
if [ -n "$ddp_backend" ] ; then ddp_backend=$ddp_backend ; else ddp_backend=no_c10d ; fi
if [ -n "$seed" ]; then seed=$seed ; else seed=0 ; fi
if [ -n "$num_workers" ]; then num_workers=$num_workers ; else num_workers=8 ; fi
if [ -n "$fp16" ]; then fp16=$fp16 ; else fp16=$(true) ; fi
...
# Optimization
if [ -n "$lr_scheduler" ]; then lr_scheduler=$lr_scheduler; else lr_scheduler=polynomial_decay; fi
...
if [ -n "$warmup_updates" ]; then warmup_updates=$warmup_updates ; else warmup_updates=960 ; fi # 960 = 0.06*16000 (0.06*total_num_updates)
...
if [ -n "$ncheckpoints" ]; then ncheckpoints=$ncheckpoints ; else ncheckpoints=10 ; fi
if [ -n "$patience" ]; then patience=$patience ; else patience=25 ; fi
...
if [ -n "$save_interval_updates" ]; then save_interval_updates=$save_interval_updates ; else save_interval_updates=400 ; fi
...
then
    ...
    grep ^S $save_dir/logs/$testlog.log | sed 's/^S-//g' | sort -nk 1 | awk 'NR % 4 == 0' | cut -f2- > $save_dir/logs/$testlog.out.src
    grep ^T $save_dir/logs/$testlog.log | sed 's/^T-//g' | sort -nk 1 | awk 'NR % 4 == 0' | cut -f2- > $save_dir/logs/$testlog.out.ref
    grep ^H $save_dir/logs/$testlog.log | sed 's/^H-//g' | sort -nk 1 | awk 'NR % 4 == 0' | cut -f3- > $save_dir/logs/$testlog.out.sys
    # score
